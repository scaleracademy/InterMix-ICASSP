...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for i, batch in enumerate(self.train_iter):
            x_array, t_array, p_array = chainer.dataset.concat_examples(batch)
            ...
            if(self.opt.BC):
                ...
                p_array = np.reshape(p_array,(self.opt.batchSize*2,-1)).astype('float32')

            x = chainer.Variable(cuda.to_gpu(x_array[:, None, None, :]))
            t = chainer.Variable(cuda.to_gpu(t_array))
            p = chainer.Variable(cuda.to_gpu(p_array), requires_grad=False)

            self.model.cleargrads()
            y , t = self.model(x, t, p, self.opt.mixup_type, self.opt.eligible, self.opt.batchSize)
            if self.opt.BC:
