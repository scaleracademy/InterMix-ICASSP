...
def mix(sound1, sound2, phase_diff, r, fs, theta):
    ...
    theta = phase_diff
    costheta = chainer.functions.cos(theta)
    term=chainer.Variable(cuda.to_gpu((2*cupy.sqrt(sound1.data*t[:,None]*sound2.data*(1-t[:,None]))*costheta.data).astype('float32')))
    sound = (sound1 * t[:,None] + sound2 * (1 - t[:,None]) + term)
    return sound

def mixup_sound(out, target_reweighted, phase, r,fs,theta):
    ...
    #indices2=[127-x for x in indices]
    #print(r.shape)
    #print(out.shape)
    #print(out[indices].shape)
    #print("out: ",out)
    phase_diff = phase - phase[indices]
    sound = mix(out,out[indices],phase_diff,r,fs,theta)
    #print(type(r))
    #r=n1/(n1+n2)
