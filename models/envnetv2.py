...
class EnvNetv2(chainer.Chain):
    def __init__(self, n_classes):
        ...
        self.theta=chainer.Parameter(shape=(1,))

    def __call__(self, x, target = None, phase=None, mixup_type = 'sound', eligible = None, bs = None):
        if eligible is not None:
            layer_mix = random.choice(eligible)
        else:
            layer_mix = None
        ...
        if layer_mix == 0:
            ...
            x, target = U.mixup_sound(x,target,phase,r,44100,self.theta)
            x = F.reshape(x, shape)
        h = self.conv1(x, self.train)
        h = self.conv2(h, self.train)
        h = F.max_pooling_2d(h, (1, 64))
        ...
        if layer_mix == 1:
            if mixup_type == 'sound':
                ...
                h = F.reshape(h, (h.shape[0],-1))
                r = cupy.random.rand(bs*2).astype('float32')
                h, target = U.mixup_sound(h,target,phase,r,44100,self.theta)
                h = F.reshape(h, shape)
            elif mixup_type == 'normal':
                shape = h.shape
                h = F.reshape(h, (h.shape[0],-1))
